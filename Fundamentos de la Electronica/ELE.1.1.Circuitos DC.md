...
# ELE.1.1. Circuitos DC 

## 1.1. Corriente Electrica

Es el flujo de carga electrica por unidad de tiempo, se mide en amperios $A$

- **Corriente Continua (DC, $I$):** Valor constante en el tiempo.

- **Corriente Alterna (AC, $i$ o $i(t)$):** Valor varía en el tiempo.

### 1.1.1. Materiales según su Conductividad:
...
**La diferencia de potencial o tension** entre dos puntos A y B en un circuito es el trabajo realizado para mover la carga entre estos 2 puntos y se mide en Voltios $V$.

$$
V_{ab} = V(t) = V_A - V_B = \frac{dw}{dq}
$$

> 	La notacion es $V$, o $u$ en Europa para valores constantes, y $v(t)$ o $u(t)$ para valores variables

### 1.2.1. Calculo del Trabajo

El trabajo diferencial $dw$ (en Joules $J$) para mover un diferencial de carga $dq$ entre dos puntos A y B, cuyos potenciales electricos son $V_a$ y $V_b$ respectivamente. es: 
...
- **Potencia Instantanea ($p(t)$):** es la cantidad de energia intercambiada en la unidad de tiempo. Se mide en Vatios $W$ 

$$
p(t)= \frac{dw(t)}{dt} = \frac{dw(t)}{dq(t)} \cdot \frac{dq(t)}{dt}=u(t) \cdot i(t)
$$

- **Energia ($w(t)$):** Integral de la potencia a lo largo del tiempo 
 
$$
w(t)=\int_{-\infty}^t  p(t)dt
$$

> Si $p(t) > 0$ se recibe o absorbe potencia (consume energia)
> Si $p(t) < 0$ se produce o entrega potencia (entrega energia)

# 2. Elementos de Circuitos Electricos

Un circuito es un conjunto de elementos interconectados que, ante una excitacion (entrada), generan una respuesta (salida), con el fin de entregar o recibir energia. Es decir, que los elementos estan combinados de tal forma de que exista la posibilidad de que se origine una corriente electrica
...
### 2.2.2. Condensador o Capacitor ($C$)
